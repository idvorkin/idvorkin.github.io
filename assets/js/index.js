...
Try these shortcuts:
  ...
  `,document.head.appendChild(t)})(),"loading"===document.readyState?document.addEventListener("DOMContentLoaded",()=>{I(e)}):I(e)}()}if("undefined"!=typeof $&&$.fn&&$.fn.ready?$(document).ready(U):"undefined"!=typeof document&&("loading"===document.readyState?document.addEventListener("DOMContentLoaded",U):U()),"undefined"!=typeof window&&window["@algolia/autocomplete-js"]){let n=window["@algolia/autocomplete-js"];e=n.autocomplete,t=n.getAlgoliaResults}function G({item:e,createElement:t}){return t("div",{dangerouslySetInnerHTML:{__html:function(e){try{let t=e.url;e.anchor&&(t+=`#${e.anchor}`);let n=e._highlightResult;n||console.log("No Highlight",e);let o=n.title.value,l=n?.content?.value??"";return`
           <span onClick="window.location='${t}';">
              <b> <a href="${t}">${o}</a></b> <span>${l}</span>
           </span>
        `}catch(t){console.log("Error in hitTemplate",t,e)}return"invalid HTML"}(e)}})}async function W(){let e=o(Object.entries(await s()).map(e=>e[1]));return{title:e.title,url:e.url,description:e.description}}async function q(e=4){try{let t=await s();return Object.entries(t).map(([e,t])=>({url:e,title:t.title||e,description:t.description||"",doc_size:t.doc_size||0,last_modified:t.last_modified||""})).filter(e=>e.description&&""!==e.description.trim()&&e.title&&""!==e.title.trim()).sort((e,t)=>e.last_modified&&t.last_modified?new Date(t.last_modified).getTime()-new Date(e.last_modified).getTime():t.doc_size-e.doc_size).slice(0,e)}catch(e){return console.error("❌ Error loading recent posts:",e),[]}}async function Y(e=3){return{sourceId:"random_posts",async getItems(){let t=Array(e).join("_").split("_");return await Promise.all(t.map(async e=>W()))},getItemUrl:({item:e})=>e.url,templates:{item:({item:e,createElement:t})=>t("div",{dangerouslySetInnerHTML:{__html:`
            <span onClick="window.location='${e.url}';" >
           <b> <a href="${e.url}">${e.title}</a></b>
            <span>${e.description}</span>
            </span>
            `}}),header:({createElement:e})=>e("div",{dangerouslySetInnerHTML:{__html:"<i style='color:grey'>Random posts ...</i>"}})}}}async function Q(e=4){return{sourceId:"recent_posts",getItems:async()=>await q(e),getItemUrl:({item:e})=>e.url,templates:{item:({item:e,createElement:t})=>t("div",{dangerouslySetInnerHTML:{__html:`
            <span onClick="window.location='${e.url}';" >
           <b> <a href="${e.url}">${e.title}</a></b>
            <span>${e.description}</span>
            </span>
            `}}),header:({createElement:e})=>e("div",{dangerouslySetInnerHTML:{__html:"<i style='color:grey'>Recent posts ...</i>"}})}}}async function V(n,o,l,i,r,a=3,s=4,c=3){if(!e){console.error("Autocomplete is not defined");return}let d=algoliasearch(n,o),u=await Y(c),p=await Q(s),f=i.startsWith("#")?i:`#${i}`;if(0===$(f).length){console.log("No autocomplete element found","autocomplete_id",i);return}return e({container:f,placeholder:"Search Igor's Musings ...",getSources:function({query:e}){let n=0===e.length;n&&(e=" ");let o=function(e,n,o,l=3,i=!1){let r="NOT tags:family-journal";return i&&(r=""),{sourceId:"featured_posts",getItems:()=>t?t({searchClient:e,queries:[{indexName:n,query:o,filters:r,params:{hitsPerPage:l,highlightPreTag:"<span style='background:yellow'>",highlightPostTag:"</span>"}}]}):(console.error("getAlgoliaResults is not defined"),[]),templates:{item:G,header:({createElement:e})=>e("div",{dangerouslySetInnerHTML:{__html:"<i style='color:grey'>Featured posts ...</i>"}})},getItemUrl({item:e}){let t=e.url;return e.anchor&&(t+=`#${e.anchor}`),t}}}(d,l,e,n?a:10,r);return n?[o,p,u]:[o]},debug:!1,openOnFocus:!0,detachedMediaQuery:""})}class Z{constructor({name:e,value:t=25,children:n=[]}){this.name=e,this.children=l(n),this.value=t}}function K(e=ee,t=X){let n=e();for(let e of n.keys())t(e,n.get(e))}function X(e,t,n=$,l=r){let i=n('<div class="alert alert-primary" role="alert"/>');n(e).after(i),l(i,()=>`<span>${o(t)}</span><span style="float: right; cursor: pointer;" title="Click for another prompt">\u{1F504}</span>`)}function ee(e=$){let t=e("h3").first(),n=t,o=[],l=new Map;for(let i=t;0!==i.length;i=e(i).next()){if("H3"===i.prop("tagName")){l.set(n,o),n=i,o=[];continue}"UL"===i.prop("tagName")&&(o=Array.from(e(i).find("li")).map(t=>e(t).text()))}return l.set(n,o),l}function*et(e){if(!e)return;let t=[];for(t.push([e,null]);t.length>0;){let[e,n]=t.shift();for(let n of e.children??[])t.push([n,e]);yield[e,n]}}function en(e=ee){return new Map(Array.from(e().entries()).map(([e,t],n)=>[e.text(),t]))}function eo(e,t,n){let[l,i]=Array.from(et(t)).find(([t,n])=>t.name===e);return o(Array.from(et(l)).map(([e,t])=>e).filter(e=>n.has(e.name)).flatMap(e=>n.get(e.name).map(t=>`${e.name}: ${t}`)))}async function el(e,t,n,o=$,l=Plotly){if(!l){console.error("Plotly is not available");return}let i=function(e){let t=Array.from(et(e)).map(([e,t])=>[e.name,t?.name]);return{ids:t.map(([e,t])=>e),labels:t.map(([e,t])=>e),parents:t.map(([e,t])=>t)}}(n),r={type:"sunburst",outsidetextfont:{size:20,color:"#377eb8"},hoverinfo:"none",marker:{line:{width:2}},maxdepth:2,displayModeBar:!1};Object.assign(r,i),r.values=void 0;try{let i=await l.newPlot(e,[r],{margin:{l:0,r:0,b:0,t:0},sunburstcolorway:["#636efa","#ef553b","#00cc96"]},{displayModeBar:!1}),a=e=>{o(`#${t}`).text(e)};return o(`#${t}`).first().click(()=>{let e=o("#sunburst text:first").text(),t=eo(e,n,en());a(t)}),i.on("plotly_sunburstclick",e=>{let t=e.points[0].label,o=eo(t,n,en());a(o)}),i}catch(e){return console.error("Failed to create sunburst plot:",e),null}}function ei(e){let t=$("<div/>"),n=`<h4> <a href='${e.url}'}>${e.title}</a></h4>`;t.append(n);let o=e.thumbnail.replace("s72-c","s320"),l=new Date(e.published),i=`
   ...
   `;return""!==e.thumbnail?(console.log(e.title),console.log(l),t.append(`
      ...
      ${i}
      </div>`)):t.append(i),t.html()}function er(e){if(console.log("Processing",e.length,"posts"),!e){console.log("No posts being imported");return}r("#random-post",()=>ei(o(e))),r("#achievment",()=>ei(o(e.filter(e=>e.title.toLowerCase().includes("achievement"))))),r("#random-recent",()=>ei(o(e.filter(e=>e.tags.includes("family-journal")))))}function ea(){$.getJSON("/ig66/ig66-export.json",er)}function es(e){$.getJSON("/eulogy.json",t=>(function(e,t){if(!t){console.log("No roles being imported");return}console.log("Processing",t.roles.length,"roles"),r(e,()=>(function(e){let t=e.title.replace(/ /g,"%20"),n=["igor","ammon"],o=n[Math.floor(Math.random()*n.length)];return`
  <div>
  ...
  `})(o(t.roles)))})(e,t))}class ec{get_tree(){return new Z({name:"7H ",children:[new Z({name:""}),new Z({name:"Be Proactive"}),new Z({name:"Begin with the end in mind"}),new Z({name:"First things First"}),new Z({name:"Think Win/Win"}),new Z({name:"First Understand"}),new Z({name:"Synergize"}),new Z({name:"Sharpen the Saw"})]})}}class ed{get_tree(){let e=new Z({name:"Health",children:[{name:"Physical"},{name:"Emotional"},{name:"Cognative"}],value:31}),t=new Z({name:"Magic",children:[new Z({name:"Card Magic"}),new Z({name:"Coin Magic"}),new Z({name:"Band Magic"})]}),n=new Z({name:"Hobbies",children:[new Z({name:"Biking"}),new Z({name:"Tech"}),new Z({name:"Juggling"})]}),o=new Z({name:"Relationships",children:[new Z({name:"Zach"}),new Z({name:"Amelia"}),new Z({name:"Tori"}),new Z({name:"Friends"})]}),l=new Z({name:"Joy",children:[new Z({name:"Balloons"}),new Z({name:"Joy to Others"})]});return new Z({name:"Invest in",children:[e,t,n,o,l]})}}function eu({url:e,title:t,description:n}){let o=`<a href='${e}'}>${t}</a>`,l=`audio_player_${Math.floor(1e10*Math.random())}`,i=e.replace(/\//g,"_");return`
    <div>
        <audio id='${l}'>
  ...
  `}async function ep(e=s,t=o){try{let n=await e(),o=Object.entries(n).map(e=>e[1]),l=t(o);return eu({url:l.url,title:l.title,description:l.description})}catch(e){return console.error("Error generating random post HTML:",e),"<div>Could not load random post</div>"}}function ef(e="#e1",t="#e2",n="#e3",o=es){try{o(e),o(t),o(n)}catch(e){console.error("Error loading random eulogy:",e)}}function eg(e=el,t=K,n=ea,o=es,l=r){try{e("sunburst","sunburst_text",new ed().get_tree()),t(),n(),o("#random-eulogy-role"),l("#random-blog-posts",async()=>await ep())}catch(e){console.error("❌ Error loading enjoy page:",e)}}function em(e=el,t=K){try{e("sunburst","sunburst_text",new ec().get_tree()),t()}catch(e){console.error("Error loading 7 habits page:",e)}}function eh(e=ea){try{e()}catch(e){console.error("Error loading IG66 page:",e)}}function ey(e=eD,t=ev,n=ew){try{e("balance-heatmap-rest"),t("balance-heatmap-work"),n("balance-radar-map-ideal")}catch(e){console.error("Error loading balance page:",e)}}let eb=["Jan","Feb","Mar","Apr","May","June","July","Aug","Sep","Oct","Nov","Dec"],e$="#00BF00";async function ew(e,t){if(void 0!==t&&t)try{await t.newPlot(e,[{type:"scatterpolar",r:[8,8,8,5,8,8,8],theta:["Work","Tech","Health","Hobbies","Relationships","Magic","Work"],name:"2020 Goal",fill:"toself"},{type:"scatterpolar",r:[7,7,5,5,5,9,7],theta:["Work","Tech","Health","Hobbies","Relationships","Magic","Work"],name:"2020 Actual",fill:"toself"}],{polar:{radialaxis:{visible:!0,range:[0,10]}},showlegend:!0},{displayModeBar:!1})}catch(e){console.error("Error creating radar map:",e)}else console.warn("Plotly is not defined, skipping chart rendering")}async function ev(e,t){let n=["Tech","Work"],o={height:20*n.length+100,margin:{t:5},pad:0},l=[{colorscale:[[0,"darkblue"],[.4,"blue"],[.5,e$],[.6,"darkred"],[1,"red"]],zmin:0,zmax:10,x:eb.slice(2,13),y:n,z:[[7,4,7,8,2,4,2,3,2,8],[10,7,5,5,3,5,6,6,7,5]],type:"heatmap"}];if(void 0!==t&&t)try{await t.newPlot(e,l,o,{displayModeBar:!1})}catch(e){console.error("Error creating work balance chart:",e)}else console.warn("Plotly is not defined, skipping chart rendering")}async function eD(e,t){let n=["Health","Hobbies","Family","Magic"],o=[{colorscale:[[0,"red"],[.4,"darkred"],[.5,e$],[.6,"blue"],[1,"darkblue"]],zmin:0,zmax:10,x:eb.slice(2,13),y:n.reverse(),z:[[4,4,3,4,5,3,2,2,3,2],[4,4,3,4,5,4,4,2,4,5],[2,3,3,4,1,5,4,3,2,4],[5,5,5,4,5,5,4,5,4,5]].reverse(),type:"heatmap"}],l={height:20*n.length+100,margin:{t:5},pad:0};if(void 0!==t&&t)try{await t.newPlot(e,o,l,{displayModeBar:!1})}catch(e){console.error("Error creating rest time chart:",e)}else console.warn("Plotly is not defined, skipping chart rendering")}$(document).ready(()=>{i(U);let e=()=>{let e=$("#search-box");e.length>0&&e.focus()};"undefined"!=typeof Mousetrap&&Mousetrap.bind("s",()=>e()),$("#toc-content").length>0&&console.log("TOC initialized"),$("#search-box").length>0&&console.log("Search initialized");let t=["item1","item2","item3"];console.log("Random item:",o(t)),console.log("Shuffled items:",l([...t])),s().then(e=>{console.log("Links loaded, count:",Object.keys(e).length)}),console.log("Blog JavaScript initialized")});export{o as random_from_list,l as shuffle,r as append_randomizer_div,i as defer,s as get_link_info,U as load_globals,n as MakeBackLinkHTML,V as CreateAutoComplete,g as initRecentAllPosts,K as add_random_prompts,el as add_sunburst,Z as TreeNode,eg as load_enjoy2,em as load_7_habits,eu as makePostPreviewHTML,eh as load_ig66,ey as load_balance,ef as load_random_eulogy};
