...
${c}`);let u="MacIntel"===navigator.platform&&navigator.maxTouchPoints>1,p=/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent)||u;if(navigator.share&&p)try{return await navigator.share({title:s,text:d,url:r}),console.log(`\u{1F4F1} Shared via native share: ${r}`),!0}catch(e){console.log("Share cancelled or failed, falling back to clipboard",e)}let m=r;return c&&(m=`From: ${a} ...

${c}

${r}`),await navigator.clipboard.writeText(m),console.log(`\u{1F4CB} Copied to clipboard with preview: ${m.substring(0,100)}...`),!1}catch(n){console.error("Failed to share/copy header link:",n);try{let n=window.location.href,o=n.includes("#")?n.replace(/#.*/,`#${e}`):`${n}#${e}`,i=I(o,t),r=`https://tinyurl.com/igor-blog/?path=${encodeURIComponent(i)}`,l=document.getElementById(e),a=l?Array.from(l.childNodes).filter(e=>e.nodeType===Node.TEXT_NODE).map(e=>e.textContent?.trim()).join(" ").trim():"",s=B(e),c=r;s&&(c=`From: ${a} ...
