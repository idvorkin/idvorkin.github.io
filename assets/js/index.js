...
Try these shortcuts:
  ...
  `,document.head.appendChild(t)}(),"loading"===document.readyState?document.addEventListener("DOMContentLoaded",()=>{I(e)}):I(e))}(),z())}function U(e){let t=$("<div/>"),n=`<h4> <a href='${e.url}'}>${e.title}</a></h4>`;t.append(n);let o=e.thumbnail.replace("s72-c","s320"),r=new Date(e.published),i=`
   ...
   `;return""!==e.thumbnail?(console.log(e.title),console.log(r),t.append(`
      ...
      ${i}
      </div>`)):t.append(i),t.html()}function V(e){if(console.log("Processing",e.length,"posts"),!e){console.log("No posts being imported");return}a("#random-post",()=>U(o(e))),a("#achievment",()=>U(o(e.filter(e=>e.title.toLowerCase().includes("achievement"))))),a("#random-recent",()=>U(o(e.filter(e=>e.tags.includes("family-journal")))))}function Y(){$.getJSON("/ig66/ig66-export.json",V)}function Q(e){$.getJSON("/eulogy.json",t=>(function(e,t){if(!t){console.log("No roles being imported");return}console.log("Processing",t.roles.length,"roles"),a(e,()=>(function(e){let t=e.title.replace(/ /g,"%20"),n=["igor","ammon"],o=n[Math.floor(Math.random()*n.length)];return`
  <div>
