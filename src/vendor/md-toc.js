  ...
  Toc.prototype._createTocContent = function recursiveToc(
    ...
    for (var element; (element = titleElements.shift()); ) {
      ...
      if (elementTag == iterTag) {
        ...
        content += `<li><a class="nav-link dropdown-item" href="${id}" title="${elementTitle}">${elementText}</a>`;
        ...
        !recurse &&
        titleNames.indexOf(elementTag.toLowerCase()) > level
      ) {
        recurse = true;
        if (!openTag) {
  ...
  Toc.prototype._showToc = function() {
    ...
    if (this.tocTop > -1) {
      window.onscroll = function() {
        ...
        if (t < self.tocTop) {
          self.toc.setAttribute(
            "style",
            "position:absolute;top:" + self.tocTop + "px;"
          );
        } else {
