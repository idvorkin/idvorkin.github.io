...
- **Why I missed it**: 7 habits — [things are created twice](/end-in-mind), once in the design (first creation) and once in the build (second creation). When the second creation is nearly free, the friction that used to force you back to the first creation disappears. You can sharpen the machete forever — more attempts, tighter eval, prettier algorithm — and never look up to ask whether you're hacking through the wrong forest. Great execution; no strategy. Manager move, not a leader move.
