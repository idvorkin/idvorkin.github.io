...
**[/time-allocation](/time-allocation)** — Igor asked how his time is actually allocated and expected "work versus family." What he got was [five buckets](/time-allocation#the-five-buckets): four he invests in on purpose (Tech, Identity (Non-Tech), Health, Relationships) and a fifth he had never named, Consumption (Non-Enjoyment), which takes whatever the other four don't. The bucket's label is Brooks' distinction: pleasure is the raw solitary hit, enjoyment is pleasure plus people and memory, and "a beer alone on the couch and a beer at a table with friends are the same beer and different hours." The part that bothered him enough to write it down: [freed time rolls downhill](/time-allocation#freed-time-rolls-downhill). Shrinking tech doesn't grow relationships, it grows consumption, because the phone has negative starting energy and enjoyment needs a text, a calendar, and somebody else's yes. "Freed hours don't get allocated, they get defaulted." Two of the good buckets [have ceilings](/time-allocation#two-of-the-good-buckets-have-ceilings): relationships is rate-limited by other people's availability ("past it, the hours I add aren't relationship hours, they're me hovering"), health saturates ("the second hour at the gym doesn't buy twice the health. Often it buys an injury"). Identity is the only bucket with room, and it has the highest activation energy. Lined up against [Escape Artists](/addiction), the taxonomy is tidy in a way Igor doesn't love: [compulsion lives in tech, addiction lives in consumption](/time-allocation#compulsion-lives-in-tech-addiction-lives-in-consumption), so "shrink tech" cuts the passion, not the addiction, and hands the hours to the leak. Closes with a warning that the whole diagram might be drawn wrong: tech may be an identity, not a bucket, "the same kind of noun as magician or dad, that got very good at feeding itself." Seven review commits followed Igor's annotations on the preview: consumption renamed to name it as procrastination, the Brooks section cut to one paragraph, the worksheet section dropped, the open question turned into a warning. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/2be21d446)

- **Interactive bucket chart** - One stacked bar of a 16-hour waking day, weekday/weekend toggle, a "shrink tech by N hours" slider, and an expect/usual toggle for where the freed hours land. The two ceilings are drawn on the bar ("their time" for relationships, "enough" for health) and hours that run past either roll into consumption; the dashed line only draws when the bucket is at its ceiling, since off it the line read as the next bucket's limit. Clicking Tech opens it into work, side projects, and enabling environment (weekday 9h = 7/1/1), with the split apportioned by largest remainder so the parts always add back up exactly. Hours are labeled as illustrative round numbers. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/ff9932773) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/ac46a83cb)
- **Cross-links** - [/addiction](/addiction#examples-from-my-life) now points at the buckets ("sorting the activity" vs "sorting the hours"), and [/build-life-you-want](/build-life-you-want#pleasure-vs-enjoyment) gains a third conversion move that isn't Brooks': **add who you want to be.** Twenty minutes of magic clips versus twenty minutes doing the trick badly start from the same pull; one makes you a magician. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/f885aa465)

### AI Orchestrators: What, Why, and How We Got Here (new post!)

**[/ai-orchestrator](/ai-orchestrator)** — "An orchestrator is everything around the agent." The post is deliberately short; the interactive building-block widget carries the story. Ten breaks, each one adding a block: one agent in a terminal → it forgets (a **work ledger** outside the agent) → it is serial (**workers**, the front door only hands out work) → workers collide (**a worktree per worker**) → you cannot see them (**a terminal per worker** that outlives its spawner) → which agent, who has quota? (**a dispatcher and a quota meter**) → nobody is watching (**a supervisor**, a dumb script that wakes the front door on done or stuck) → done is a lie (**a quality gate and a merge path**, findings go back to the worker not to you) → chat is the wrong shape for review (**rich review**, a page you can annotate) → no one view of the whole (**an overview** board). Plus a table of who fills each block across Gas City, firstmate, native Claude, and Igor's own. Igor runs two: [Larry](/larry) on his hand-rolled stack, [Gas City](/gas-city) when he wants one off the shelf. [Why hand-roll](/ai-orchestrator#why-i-hand-roll-mine): hyper-tuned to him, he learns how each block works by building it, and every block can come from a different best-in-breed third party. The bet: [Claude will absorb most of it](/ai-orchestrator#claude-will-probably-absorb-most-of-this). "Every block is a gap in the product, and products close gaps. Worktree isolation was my own block for a while and then it turned up as a flag." Which is the whole reason to keep each block cheap enough to delete. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/e462fbf19)

- **Viewer bake-off** - Igor's preview note: title block adds nothing, the 0-to-9 dot stepper is clunky, should the story sit above the chart? Three working viewers went on one pick sheet (play-and-scrub, click-the-drawing, scroll-driven), the story moved to `_data/orchestrator_blocks.yml` so there's one copy behind every variant. He picked play-and-scrub, then asked for Back/Next with the controls up top; the story block carries a runtime-measured height floor so nothing moves as the copy changes length. Then: pressing Play should not start you at the end, so the whole stack became the scrubber's first slot and Next walks into the first break. Codex handled the final passes: dropped the decorative LEGO studs, and reflowed the walkthrough for phone screens with container-aware geometry and right-side layer labels. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/4d15ca69c) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/cd9f4e487) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/963295f9c) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/03d97cf68)
- **Cross-links** - [/how-igor-chops](/how-igor-chops#the-8-stages-of-ai-coding) (Stage 8 is made of these bricks), [/gas-city](/gas-city) (one filling of the bricks every orchestrator ends up with), [/larry](/larry) (his own hand-rolled orchestrator), [/ai-cockpit](/ai-cockpit) (the cockpit is the overview brick), and [/wally](/wally). [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/e22ab60c0)

### How I Manage AI Tokens (new post!)

**[/token-management](/token-management)** — "I rent the most expensive brain I can get" stopped being the whole answer: Igor can burn through a \$200/month plan, and the model he actually wants runs out before the plan around it does. The post opens with [OpenRouter's price list](/token-management#tokens-are-not-one-price) as of September 18: \$10/\$50 per million for `claude-fable-5.1` and `gpt-6-astra` at the top, \$0.10/\$0.20 for `muse-spark-1.3-contributor` at the bottom, a 100x spread on input and 250x on output for the same job in the same repo. [Why buying the best stopped working](/token-management#why-buying-the-best-stopped-working): a plan isn't one meter. That morning the Claude week sat at 69% remaining with 48% of the window gone, but the top model's own weekly sub-limit was already down to 57%. "When I'm typing, I'm the rate limiter. When agents are running unattended, nobody is." The fix is routing, and you can't route without numbers, so [Step 1 is quota-axi](/token-management#step-1-measure-with-quota-axi), Kun Chen's tool that reads quota windows straight out of the CLI credentials on disk (Claude Code, Codex, Cursor, Copilot, Grok) with no keys to paste, replacing a 40,000-token `/usage` dialog capture with a one-second subprocess. [Step 2: read pace, not percentage](/token-management#step-2-read-pace-not-percentage). Codex was 76% left with 87% elapsed, Grok 65% left with 93% elapsed; subscription windows are use-it-or-lose-it, so big jobs go there first. "Look at percent remaining alone and you'd read 69, 76, 65 and pick the biggest one. That's backwards." The numbers now live as a page in [the Cockpit](/token-management#put-it-where-you-already-look) with a SPEND flag on any window resetting within 24 hours with more than half unspent. And an unhedged endorsement of the [Muse contributor tier on public repos](/token-management#muse-contributor-on-public-repos): "oh my god, you should," with the catch that Meta trains on the prompts, which on a public repo are the already-public diff. Written three times: first as a Larry post, then reframed FYI-style with the price spread first, then rewritten in Igor's first person after his review ("this sounds terrible, like it's been written by Larry"). The source file was renamed from `token-management.md` to `ai-quota-meter.md` because the original matched the `*token*` secrets rule in `.gitignore`. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/25aeb037c) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/027e0d71e) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/96e93e4de)

### Infrastructure & CI (2026-09-21)

- **pr-checks skill** - `content_guidelines.md` defined the Architect/Carpenter/Judge/Workflow grid but nothing produced it; every review re-derived it by hand. The new `.claude/skills/pr-checks/` prints exactly that format for the posts changed on a branch. Anything a regex can settle is code (front-matter, internal-links, opening, alerts, images, books, ai-slop, rebased), so the grid works offline with no API key; only headers, voice, and ai-patterns go to the Jev decision model. Calibration says one of those three can be trusted to fail a build and two cannot, so ai-patterns is blockable behind `--block-jev` and the other two never block. Igor then asked to double the checks: every code check is also put to Jev, and `calibrate.py --doubled` scored it against the code verdict: alerts/ai-slop/images 100%, books/opening 95%, internal-links 76%, front-matter 52%. "Jev finds what is present in the text and misses what is absent from it": a missing `tags:` key it never once noticed (0/5 injected). Two calls per post, about \$0.0004, half a second. Wired as the `blog-checks` no-mistakes gate. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/18755cb49) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/a33547889)
- **no-mistakes gate config** - `.no-mistakes.yaml` lands with targeted build and fast-test validation, prose-aware review rules, and a test runbook that avoids the shared-port Playwright trap. Agent order flipped to Grok first: the Grok subscription has the most headroom and blog content is public, so there's no privacy reason to keep gate runs off it. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/5432d81ec) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/804c6b83f)
- **Dev banner links the changed pages** - The dev-server banner showed branch, PR, and port. It now also lists the permalinks of the posts the branch touches (committed against the merge-base plus uncommitted edits), each a link, so a multi-page preview is one click from any of its pages. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/94a5768f2)
- **AGENTS.md is the instruction file** - `CLAUDE.md` is now a symlink to it. One file for every agent, minus the mandatory `bd dolt push` close-out step that could only fail since this repo's beads store has no Dolt remote. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/11accd59c)
- **Small fixes** - The `?` shortcut help now lists the annotate-mode toggle (Ctrl/Cmd+Shift+A), which had lived in the annotation tool since it shipped but was never discoverable. Two historic changelog anchors repaired; anchor-checker now passes clean across 364 files and 6,248 links. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/f02fec9eb) [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/4c9d3de49)

### chop-conventions (2026-09-21)

**[chop-conventions](https://github.com/idvorkin/chop-conventions)** (shared CHOP skills & conventions)

The `muse-herdr` skill now requires a code review of every Muse change before merging. The trigger: three Muse branches merged on green rungs, each needed a compile fix, one carried three behavior bugs a later review found, and one starved a feature on first use. Igor's instruction, verbatim in the commit: "From now on, you need to code review Muses' changes. Especially any changes to stories, documents, or architecture. They get a lot shittier." New rules: docs and stories are written by the manager and a Muse's doc diff is rewritten; the code gets a read-only reviewer agent before merge; expect a compile fix per branch since Muses cannot build across modules; read `git log main..branch` before merging. [<i class="fa fa-github"></i>](https://github.com/idvorkin/chop-conventions/commit/fc4254267)

### Other Projects (2026-09-21)

**[exercise-analyzer](https://github.com/idvorkin/exercise-analyzer)** (iPhone exercise form analyzer)

Thirty commits, most of them Igor's requests from the gym. Split squats with both feet on the floor are now recognized, counted, and scored: a barbell set had been read as swings with no reps, and the plate hides the head from the side, so the analyzer tracks hip height over the lower foot in leg lengths instead. Pull-ups learn a new grip's torso scale the way they learn its bar line. A deleted set stays deleted when a pass on it finishes afterwards. The workout page got pinch-to-zoom and drag-to-pan on the chart ("only zoom when each line ain't thick enough": on a phone-wide plot a 25-second set was a 3-point band), a sideways swipe pans instead of navigating back, and a folded day now says what was done as sets × reps with the exercise drawings. Mid-workout, the Workouts tab opens on the running workout. The wrist-to-phone link now sends a heartbeat a second and logs every gap, after "losing the watch is terrible." README rebuilt with screenshots of form, workouts, and recovery. [<i class="fa fa-github"></i>](https://github.com/idvorkin/exercise-analyzer/commit/6d617d87c) [<i class="fa fa-github"></i>](https://github.com/idvorkin/exercise-analyzer/commit/307099851) [<i class="fa fa-github"></i>](https://github.com/idvorkin/exercise-analyzer/commit/0fed44bb0)

**[quota-axi](https://github.com/idvorkin-ai-tools/quota-axi)** (LLM subscription quota meter for agents)

The tool behind the token post grew providers all week: Antigravity via CLI `/quota` with Gemini and Claude/GPT windows as separate jointly-bounding groups, a read-only Command Code provider reading five-hour and weekly windows from whoami and credits endpoints, Z.AI coding-plan and OpenCode Go contributor-package quota, each Pi openai-codex account reported separately, and `CLAUDE_CODE_OAUTH_TOKEN` as a credential source. Fixes: Claude token expiry confirmed after rate-limited reads, Copilot quota read from the GitHub CLI login when apps.json can't answer, the proxy dispatcher paired with its own undici fetch. Releases 0.1.44 through 0.1.46. [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/quota-axi/commit/4b91f9e90) [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/quota-axi/commit/96d724202)

**[lavish-axi](https://github.com/idvorkin-ai-tools/lavish-axi)** (HTML artifact review tool)

Structured conversation history: agent replies used to render as one escaped block, and reviewer notes queued on elements vanished the moment they were sent, so the dialog read as one-sided. A new pure module renders a deliberate Markdown subset server-side and the annotation history joins the transcript. Also: an opt-in revision legend for agent-declared artifact edits, an explanation playbook for existing systems (PRs, incidents, architecture) since the router used to land on plan or comparison, an opt-in Herdr readiness chime, recoverable live-review failures, and bounded stored transcripts. Releases 0.1.68 through 0.1.76. [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/lavish-axi/commit/c95f3c474) [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/lavish-axi/commit/b4e82c635) [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/lavish-axi/commit/a3504db27)

**[Settings](https://github.com/idvorkin/Settings)** (dotfiles & tools)

- **`y` window switcher** - Clockwise window-number overlay, focus and close windows by displayed number, focus when the action is omitted, nested focus and terminal completion, deferred GUI imports for startup, then the whole thing forwarded to its Alfred implementation with 3,600 lines of Python removed. [<i class="fa fa-github"></i>](https://github.com/idvorkin/Settings/commit/653134680) [<i class="fa fa-github"></i>](https://github.com/idvorkin/Settings/commit/d5f1d9bc6)
- **`caff`** - Battery-aware caffeinate wrapper driven by Herdr agents. Plain caffeinate keeps a laptop awake until the battery is flat; caff re-evaluates every tick from `pmset -g batt` and `herdr agent list`: full caffeinate when plugged in, idle-only on battery while agents work, nothing plus a scheduled wake when all agents are idle, normal sleep at the floor. Follow-up hardened it to fail toward sleep, not awake. [<i class="fa fa-github"></i>](https://github.com/idvorkin/Settings/commit/0e6944507) [<i class="fa fa-github"></i>](https://github.com/idvorkin/Settings/commit/275f5cee7)
- **Dead API keys dropped from zsh** - A subscription CLI prefers an env API key over its own login: grok printed "You are using XAI_API_KEY" and ignored the grok.com session, `claude -p` hung or 401'd on `ANTHROPIC_API_KEY`. All three keys were dead when probed, so exporting them only broke the CLIs that would otherwise work. [<i class="fa fa-github"></i>](https://github.com/idvorkin/Settings/commit/615a368b7)

**[hierarchy-of-money-field-map](https://idvorkin-ai-tools.github.io/hierarchy-of-money-field-map/)** (explainer) [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/hierarchy-of-money-field-map)

New repo: Gregory Gundersen's Hierarchy of Money as an illustrated five-chapter parable (stones, IOUs, the banker, the run, fiat) plus the closing hierarchy and the cast. A problem-then-solution walkthrough followed, in the same shape as the orchestrator post: each layer exists because the one below broke, Back/Next/Play, ten breaks. Listed on [The world, explained](https://idvorkin-ai-tools.github.io/). [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/hierarchy-of-money-field-map/commit/0b5ad91be) [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/hierarchy-of-money-field-map/commit/bb0029b15)

**[autopreso](https://github.com/idvorkin-ai-tools/autopreso)** (realtime speech to presentation)

A Deepgram speech-to-text provider and an OpenRouter agent provider, shipped together because on their own neither one gets you off OpenAI. Deepgram is the first streaming engine that works off macOS, since it's a hosted WebSocket with no native sidecar; encoding is declared, never sniffed. The CLI refuses Deepgram without a key. [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/autopreso/commit/024cde234)
...
The LG 5K2K ultrawides join the 21:9 column: the 45" (LG 45GX950A) and the 52" (UltraGear evo G9 52G930B, CES 2026). Both are 5120×2160, so the 52" spreads identical pixels over seven more inches of diagonal, ~107 PPI against ~123, which is the app's point in one column. The hypothetical 45" 3:2 was dropped. [<i class="fa fa-github"></i>](https://github.com/idvorkin/monitor-explainer/commit/0c0024a08)
