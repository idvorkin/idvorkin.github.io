...
_**Partial week — through 2026-04-16 (Thu).** ~35 blog commits + substantial cross-repo activity; Fri-Sun commits will roll up in the next pass._
...
- **Quote attribution fix** — Reattributed the cryptophasia framing quote ("Assuming you crack persistence, this is going to ruin you…") from "Me, thinking about what persistent Larry actually does to me" to **IRL Bestie** — the human friend who actually first articulated the concern. Honest credit. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/92899751d)

### Changelog Skill: Bash to Python Hot-Path Migration

The [`/changelog` skill](https://github.com/idvorkin/idvorkin.github.io/blob/main/.claude/skills/changelog/SKILL.md) has accumulated enough gnarly bash (multi-stage pipelines, nested loops with serial `gh api` calls, GNU-only `date -d` math) that three hot paths earned standalone stdlib Python tools per [chop-conventions](https://github.com/idvorkin/chop-conventions/blob/main/CLAUDE.md) ("default Python for non-trivial scripts"). Each tool keeps classification/ranking logic behind a pure function for unit testing; subprocess boundaries stay thin.

- **`partial_week.py` — merge mid-week partial entries** — A mid-week changelog run would previously create a partial `## Week of YYYY-MM-DD` entry. The next mid-week run (or the canonical Monday run the following week) would create a _second_ entry for the same ISO week, fragmenting the log. New `check` / `delete` subcommands detect when the top-most week heading falls within the current ISO week and regenerate fresh. Handles both Mon-anchored partials (entry dated Mon, today is Thu) and trailing-date partials (entry dated Wed from a Wed run, today is Fri) via a single ISO-week containment check. 16 unit tests including year-boundary date math and section+TOC deletion. JSON output. Cross-platform (`date -d` is GNU-only). [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/ea932eed4)
- **`scan_repos.py` — parallel cross-repo scanner** — Replaced the nested serial bash loop (`for org; do gh repo list; while read repo; do gh api commits; done; done`) — ~30-50 sequential `gh api` calls blocking one-at-a-time — with a `concurrent.futures.ThreadPoolExecutor` fan-out (default 8 workers). Typical scan drops from tens of seconds to ~4s. `filter_active()` classification pure-function-tested; `scan()` accepts injected `_list_org_repos` / `_fetch_repo_commits` callables so tests drive the full pipeline without mocking subprocess. Failed per-repo fetches return `[]` rather than aborting the whole scan (archived/private/rate-limited repos can't poison a sweep). [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/bba3d85a7)
- **`changed_files.py` — ranked file-change frequency** — Replaced the 6-stage shell pipeline (`git log | grep | sort | uniq -c | sort -rn | head`) with one git call + in-memory counting. Adds **deterministic alphabetical tie-breaking** the bash version lacked (`sort -rn` isn't stable across ties). [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/bba3d85a7)
- **Inclusive repo scope doc** — Reframed the `/changelog` skill's "Key Repos to Track" curated table as "Scope: all public repos under both orgs." The scan was already inclusive via `gh repo list --visibility public`; the old table read like a filter whitelist and created false scope confusion. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/893d0c434)
- **Header-copy-link teardown fix** — Surfaced while trying to commit this week's changelog updates: `just fast-test` failed even though all 256 assertions passed. Root cause was a pre-existing race in `src/header-copy-link.ts:893` — `setTimeout(() => document.addEventListener(...), 100)` fires _after_ vitest / happy-dom tears down the DOM between tests, raising `ReferenceError: document is not defined` as an uncaught exception. Fixed with `if (typeof document === "undefined") return;` guard inside the timeout body. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/05a1e2e1c)
...
- **Share-link truncation: bullet-preserving, sentence-aware** ([PR #488](https://github.com/idvorkin/idvorkin.github.io/pull/488)) — The share-link popup's quote-truncation used to chop mid-sentence when the selection was too long. Rewritten to prefer sentence boundaries and preserve whole bullets rather than truncating inside one. Tests cover multiple boundary scenarios. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/736834ffe)
- **`ci(backlinks)`: force our back-links.json on push rejection** — When the backlinks-updater workflow's push gets rejected (remote moved ahead between checkout and push), force our generated `back-links.json` rather than bailing. Prevents recurring CI failures when two edits land close together. [<i class="fa fa-github"></i>](https://github.com/idvorkin/idvorkin.github.io/commit/c3a3356dc)
...
- **`harden-telegram` — pane-resolution via parent-pid chain** — Watchdog now resolves the target tmux pane by walking the parent-pid chain (via new `rmux_helper parent-pid-tree` subcommand — see Settings below) rather than querying tmux for the active pane. More robust when multiple panes are active or when the watchdog fires from a detached process. Python fallback when `rmux_helper` unavailable. Companion: a new `telegram_debug --undelivered` mode for emergency-comms polling when MCP is dead. [<i class="fa fa-github"></i>](https://github.com/idvorkin/chop-conventions/commit/cfedb6cd9)
- **`delegate-to-other-repo` — CLAUDE.md-greedy + audit trail mandate** — Skill now mandates reading every CLAUDE.md in the target repo before editing (prevents rehydrating stale rules), writing an audit trail at `docs/agent-notes/<date>-<scope>.md`, and skill-review-on-update. The greedy read is the response to cases like the "can't gh pr close" false claim below. [<i class="fa fa-github"></i>](https://github.com/idvorkin/chop-conventions/commit/dd4619a3d)
- **CLAUDE.md hardening sprint** — Batch of rules codifying this week's learnings: _mandate worktrees for all work in this repo_ (the symlink trap is real), _`rmux_helper` is the canonical environment-primitive_ (don't re-invent tmux/pid walking), _`MERGEABLE` != rebase-clean_ and _`git branch -d` refuses rebase-merged_ (cleanup gotchas), _`git check-ignore` directory-only pattern_ gotcha, _close stale PRs/issues yourself_, _mid-project `.gitignore` addition is branch-dependent_, plus two 2026-04-15 session lessons (pre-commit silent-abort and hook-scope gotchas). [<i class="fa fa-github"></i>](https://github.com/idvorkin/chop-conventions/commit/ed5078994)
- **`gen-tts` skill — Gemini 3.1 Flash TTS, bash → Python** — New text-to-speech skill backed by Gemini 3.1 Flash TTS. Shipped as bash first (`gemini-tts.sh`) then rewritten in Python (`generate-tts.py`) — curl+jq pipeline → `TTSResult` class, header-based auth, HTTP status check, retry on transient 5xx, safety-filter blocks surfaced with actionable error messages. Single + batch + prosody tags. Flags: `--style-prompt` / `--style-preset` / `--style-file`. Voice catalog: Charon (default), Freud, Soprano. Default landed on Charon after comparative tuning. The dropped `gemini-tts.sh` is a clean example of "escalate from shell to Python once it earns it." Companion `gen-stt` SKILL.md also rewritten to reflect Python-only surface. [<i class="fa fa-github"></i>](https://github.com/idvorkin/chop-conventions/commit/70b0693b9)
- **Soprano iteration — Gemini-as-critic tournament loop** — Separate follow-up where a tournament+scorecard loop used Gemini itself as judge to iterate on the Soprano voice prompt. The v2 winner scored 6/10 on Enceladus (base voice + tournament-derived prompt), adopted as the new `gen-tts` Soprano preset. Documented as a reference pattern for using an LLM as a quality-critic on its own outputs. [<i class="fa fa-github"></i>](https://github.com/idvorkin/chop-conventions/commit/b082c0daf)
...
- **`rmux_helper parent-pid-tree` — tmux pane resolution via parent chain** ([PR #76](https://github.com/idvorkin/Settings/pull/76)) — New subcommand that walks the parent-pid ancestor chain and resolves the containing tmux pane id. Includes `--tree` flag for ancestor chain details and `--json --verbose` for enriched proc details with root marker. Consumed by the `harden-telegram` watchdog (see chop-conventions above). Shipped with: extracted `TmuxProvider` and `ProcReader` traits for dependency-injection, CLI-level tests via DI, edge-case coverage for parent-pid-tree. Plus `install-completions` subcommand and dynamic pid completer for shell autocomplete. [<i class="fa fa-github"></i>](https://github.com/idvorkin/Settings/commit/2937c8486)

**[blob](https://github.com/idvorkin/blob)** (shared static assets)

- **raccoon-larry + raccoon-claw-trio illustrations** — Added a new raccoon-Larry illustration (with lobster-Freud variant) used on `/larry` to replace the older portrait, plus a raccoon-claw-trio updated with Freud-Larry. Chroma-key fix: rewrote the background-removal step to use a single-pass filter so edges come out clean on transparent backgrounds. [<i class="fa fa-github"></i>](https://github.com/idvorkin/blob/commit/2c6c7b259)
...
- **New explainer project** — Static Pages explainer of Dolt's `refs/dolt/data` trick and how it enables git-style syncing for beads across machines and agents. Narrative was rewritten mid-week from "Dolt internals" to **start from beads (the actual user problem: cross-agent / cross-machine sync), end at the GitHub gotchas**. Added PlantUML diagrams (refs topology + push flow), a scenario-7 section on fork workflow and the PR hole, a git-refs primer before the punchline, and rollback/audit content. [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/dolt-explainer/commit/464b0f5a6)

**[larry-voice-samples](https://github.com/idvorkin-ai-tools/larry-voice-samples)** (new — Larry voice catalog + iteration writeup)

- **Larry voice catalog — Gemini 3.1 Flash TTS** — New static site cataloging candidate voices for Larry. MP3 mirrors + `index.html` for inline browser playback. Per-voice pricing and latency tables. Speed variants (1.25×, 1.4×, 1.6×, 1.8×, 2.0×, 2.2×, 2.4×, 2.6×) with a global speed slider wired to `playbackRate` — live rate change with no pause/rewind, snap to common presets. Collapsible catalog sections per voice. [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/larry-voice-samples/commit/5d5dfe43d)
- **Soprano iteration loop — Gemini-as-critic writeup** — Companion page documenting the tournament+scorecard loop that hit 6/10 on Enceladus for the Soprano voice prompt. Restructured for readability: winner-first, combined v1/v2 trajectory table with inline audio players, collapsed deep-dives, framed as hill-climbing with a model-judge. The v2 winner (Enceladus base + tournament-derived prompt) is now the Soprano preset in `gen-tts`. [<i class="fa fa-github"></i>](https://github.com/idvorkin-ai-tools/larry-voice-samples/commit/8d4128a59)
