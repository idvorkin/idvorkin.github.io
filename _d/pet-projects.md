---
layout: post
title: "Pet Projects"
permalink: /pet-projects
redirect_from:
  - /tinkering
---

As an engineer, I love to build bespoke tools that solve my unique problems exactly the way I want them solved. The bonus? This is lets me learn new technologies (did you know my blog has multiplayer support?). Sometimes I even make a tool so I have something to learn from. And with AI, this list of projects and explorations has exploded! Note to self, when I want to veg out and just feed my social media addiction I should do some vibe coding on all of these.

## Self Reflection

| Project                                                                        | Description                                                                                      |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| [This Blog](https://github.com/idvorkin/idvorkin.github.io)                    | Where I think through writing. Jekyll-based enabling environment. [idvork.in](https://idvork.in) |
| [Tony the Tesla](https://github.com/idvorkin/tony_tesla)                       | AI life coach for working through problems. [Demo](https://idvork.in/bike-tesla-identity)        |
| [`ij`](https://github.com/idvorkin/nlp/blob/master/igor_journal.py)            | Journal management and analysis CLI tool                                                         |
| [`gmail`](https://github.com/idvorkin/Settings/blob/master/py/gmail_reader.py) | Used to analyze journal entries from email. [Blog post](https://idvork.in/process-journal)       |

## Personal Productivity Tools

| Project                                                                                    | Description                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Humane Tracker](https://github.com/idvorkin/humane-tracker-1)                             | Habit tracking across 5 wellness categories. Firebase-based with weekly views                                                                                                                                                                                                                                                                                                                        |
| [Weight Analysis](https://github.com/idvorkin/jupyter/blob/master/Weight%20Analysis.ipynb) | Jupyter notebook analyzing Apple Health weight data. Box plots, time series, interactive visualizations                                                                                                                                                                                                                                                                                              |
| [CLI Tools](https://github.com/idvorkin/Settings/tree/master/py)                           | [`a`](https://github.com/idvorkin/Settings/blob/master/py/a.py) (Aerospace), [`y`](https://github.com/idvorkin/Settings/blob/master/py/y.py) (yabai), [`chop`](https://github.com/idvorkin/Settings/blob/master/py/chop.py) (logs), [`ai-clip`](https://github.com/idvorkin/Settings/blob/master/py/ai_clip.py), [`tmux_helper`](https://github.com/idvorkin/Settings/blob/master/py/tmux_helper.py) |
| [OmniFocus CLI](https://github.com/idvorkin/omnifocus_cli)                                 | Command-line interface for OmniFocus task management                                                                                                                                                                                                                                                                                                                                                 |
| [`gpt`](https://github.com/idvorkin/nlp/blob/master/gpt3.py)                               | GPT integration utilities                                                                                                                                                                                                                                                                                                                                                                            |
| [`captions.py`](https://github.com/idvorkin/nlp/blob/master/captions.py)                   | Converts YouTube VTT transcripts to clean text                                                                                                                                                                                                                                                                                                                                                       |

## Computer Vision & Motion

| Project                                                      | Description                                                                             |
| ------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| [Swing Analyzer](https://github.com/idvorkin/swing-analyzer) | Browser-based kettlebell swing analyzer. [Live demo](https://swing-analyzer.vercel.app) |
| [Magic Monitor](https://github.com/idvorkin/magic-monitor)   | Smart mirror app with instant replay and AI-powered smart zoom. [Live demo](https://magic-monitor.surge.sh) |
| [Video Edit](https://github.com/idvorkin/video-edit)         | Computer vision explorations: YOLO object detection, OpenCV motion detection            |

## Dev Tools & Infrastructure

| Project                                                                    | Description                                                                                                                   |
| -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| [Chop Conventions](https://github.com/idvorkin/chop-conventions)           | Development conventions for AI-assisted coding. [Blog post](https://idvork.in/vibe)                                           |
| [VAPI Call Viewer](https://github.com/idvorkin/vapi-call-viewer)           | TUI for viewing Vapi.ai call transcripts. [Demo video](https://www.youtube.com/watch?v=hE-qimUbKdg)                           |
| [Manager Book Redirect](https://github.com/idvorkin/manager-book-redirect) | Creates shareable blog links with proper Open Graph titles. [Live service](https://idvorkin--igor-blog-fastapi-app.modal.run) |
| [Jupyter Notebooks](https://github.com/idvorkin/jupyter)                   | Data exploration notebooks. [View live](http://nbviewer.jupyter.org/github/idvorkin/jupyter)                                  |
