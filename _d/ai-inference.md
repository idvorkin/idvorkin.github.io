...
- **Speculative decoding** — a small, fast _draft_ model proposes the next several tokens; the big model then checks that whole batch in a single parallel forward pass. The asymmetry it exploits: **checking is cheaper than generating**. Verifying the batch streams the weights out of memory once — about the cost of generating _one_ token — so a draft that's right most of the time nets you ~2-3 accepted tokens per expensive big-model pass. Wrong guesses just get thrown away, so the output is provably identical to what the big model would've produced alone — a pure speedup, not a quality trade. ([How it works](https://bentoml.com/llm/inference-optimization/speculative-decoding); [why checking beats generating](https://www.salmanq.com/blog/speculative-decoding/).)
